...
function restoreMoodleFromKopereZip(array $job, ?array $target = null): array {
    ...
    try {
        ...
        if ($layout["schema_dir"] !== null && $layout["data_dir"] !== null) {
            ...
            $configsummary = restoreEnsureMoodleConfigVersion($job,$target, $layout['manifest_file']);
 ...
 * @return array
 */
function restoreEnsureMoodleConfigVersion(array $job,array $target, ?string $manifestfile = null): array {
    $pdo = restorePdo($target);
    $prefix = (string) ($target['dbprefix'] ?? 'mdl_');
