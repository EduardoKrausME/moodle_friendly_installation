...
# Moodle Friendly Installation panel installer for Ubuntu.
# This file is normally called by installation.sh.
# Usage: sudo bash installation-ubuntu.sh [repo-branch]
...
detect_os() {
    ...
    if [[ "${OS_ID}" != "ubuntu" ]]; then
        die "This installer supports Ubuntu only. Detected: ID=${OS_ID}, ID_LIKE=${OS_LIKE}. Use install-moodle-friendly-installation.sh to auto-select the correct installer."
    fi
